...
interface Shape {									// Shape 인터페이스 정의, 도형의 둘레·면적 계산 메서드를 포함
	public double perimeter();						// Shape 인터페이스에 선언된 메소드, 도형의 '둘레'를 계산하는 추상메서드, 컴파일 시 public abstract 제어자가 자동 추가
	public float area();							// Shape 인터페이스에 선언된 메소드, 도형의 '면적'를 계산하는 추상메서드, 컴파일 시 public abstract 제어자가 자동 추가
}
...
class Rectangle implements Shape {											// 직사각형 Rectangle 클래스 정의, Shape 인터페이스를 구현
	...
	public Rectangle(int x1, int y1, int x2, int y2) {						// Rectangle 클래스 생성자 메서드, 직사각형의 대각선 두 점의 좌표를 입력받아 초기화 
		// TODO Auto-generated constructor stub
		this.x1 = x1;
		this.y1 = y1;
		this.x2 = x2;
...
class Square extends Rectangle {											// 정사각형 Square 클래스 정의, Rectangle 클래스를 상속(Rectangle 클래스의 모든 멤버를 상속 받음)
		...
		public Square(int x1, int y1, int side) {							// Square 클래스 생성자 메서드, 사각형의 한 꼭지점의 좌표와 변의 길이를 입력받아 초기화 ex) (x1, y1) = (1, 2) / (x1 + side, y1 + side) = (4, 5)
...
public class 제출과제_7장_인터페이스 {
	public static void main(String[] args) {
		...
		Shape r = new Rectangle(1,2,3,4);									// Shape 인터페이스를 구현한 Rectangle 클래스의 객체를 생성
		Shape s = new Square(1,2,3);										// Shape 인터페이스를 구현한 Square 클래스의 객체를 생성
