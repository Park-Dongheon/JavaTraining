...
public class Test02_iterator {
	...
	public static void main(String[] args) {
		...
		List<String> list = new ArrayList<String>();//크기가 10인 리스트 생성			// list의 참조변수는 ArrayList<E> 객체를 가리킴 => List<String> list 형태로 사용가능
		list.add("서울");
		list.add("북경");
		list.add("상해");
		list.add("서울");
		...
		Iterator<String> iter = list.iterator();//iterator()는 iterator<String> 객체를 반환,		참조변수liter 가 list(참조변수)의 iterator()메서드 호출 => 커서(cursor == prompt)객체 생성
		//480페이지 1번째 문단 
		while (iter.hasNext()) {									// hasNext(): 현재 커서 다음에 데이터가 존재하는지 판단, next()는 커서 다음의 데이터를 반환, 커서를 다음 데이터로 이동
