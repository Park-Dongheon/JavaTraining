...
public class Test10_hashset {
	...
	* 보너스 번호는 그 중에서 추출되지 않은 숫자 중에서 하나가 선택됩니다. 즉, 보너스 번호는 일반 번호와 중복될 수 있습니다.
	...
	*/
	...
	public static void lotto_generator(int n) {
		...
		HashSet<Integer> lotto = null;			// lotto 객체가 메모리의 다른 곳을 할당 됨
		for (int i = 0; i < n; i++) {
			...
			List<Integer> L = new ArrayList<Integer>(lotto);		// 임의로 생성된 number와 보너스번호를 ArrayList에 저장
			int bonus = number.nextInt(46);
			System.out.print("보너스번호 ["+bonus+"]\n");
			Collections.sort(L);;									// number 숫자 정렬
			L.add(bonus);
